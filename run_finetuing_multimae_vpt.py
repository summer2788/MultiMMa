# This is for single modal(rgb) to multi task(depth, segmentation).
...
from utils.log_images import log_taskonomy_wandb, log_semseg_wandb
...
DOMAIN_CONF = { #REF : DomainConf
    ...
    'semseg': {
        ...
        'channels': 40, #class for nyu dataset
    },
    'mask_valid': {
        'stride_level': 1,
        'aug_type': 'mask',
...
def get_args():
    ...
    parser = argparse.ArgumentParser('MultiMAE depth-segmentation fine-tuning script', add_help=False)
    parser.add_argument('--batch_size', default=64, type=int, help='Batch size per GPU')
    parser.add_argument('--epochs', default=2000, type=int)
    parser.add_argument('--save_ckpt_freq', default=200, type=int)
    parser.add_argument('--tmp', default=False, action='store_true') #for debugging
    ...
    parser.add_argument('--drop_path_decoder', type=float, default=0.0, metavar='PCT', #percentage
                        help='Drop path rate (default: 0.0)')
    ...
    parser.add_argument('--decoder_interpolate_mode', type=str, default='bilinear',
                        choices=['bilinear', 'nearest'], help='for convnext adapter')  #for segmentation decoder 
    ...
    parser.add_argument('--aug_name', default='simple', type=str,  choices=['simple', 'nyu-augs'],
                        help='One of [simple, nyu-augs] (default: simple)')
    ...
    parser.add_argument('--dist_on_itp', action='store_true')  #set false not to use torch.distributed.launch
...
def main(args):
    ...
    additional_targets = {domain: DOMAIN_CONF[domain]['aug_type'] for domain in args.all_domains} # {'rgb': 'image', 'depth': 'mask', 'semseg': 'mask'}
    ...
    dataset_train = build_semseg_dataset(args, data_path=args.data_path, transform=train_transform)#dataset_train is a MultiTaskImageFolder object
    ...
    if args.distributed:  # args.distributed / not used 
        num_tasks = utils.get_world_size()
        global_rank = utils.get_rank()
    ...
    adapters_dict = {
        'dpt': partial(DPTOutputAdapter, stride_level=1),
    ...
    output_adapters = { #REF : output_adapters.py
        # ex DPTOutputAdapter, ConvNeXtAdapter
        domain: adapters_dict[DOMAIN_CONF[domain]['output_adapter']]( 
            num_classes=DOMAIN_CONF[domain]['channels'],
            patch_size=args.patch_size,
            main_tasks=args.decoder_main_tasks # ex ['rgb']
        )
        for domain in args.out_domains # ex ['depth', 'semseg']
    }

    model = create_model(
        args.model, # example : multivit_base
    ...
    if args.eval: # skip train process
        val_stats = evaluate(model=model, tasks_loss_fn=tasks_loss_fn, data_loader=data_loader_val,
                             device=device, epoch=-1, in_domains=args.in_domains,num_classes=args.num_classes, mode='val', log_images=True,
                             return_all_layers=return_all_layers, standardize_depth=args.standardize_depth, fp16=args.fp16)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            log_images=log_images,fp16=args.fp16
        )
...
def train_one_epoch(model: torch.nn.Module, tasks_loss_fn: Dict[str, torch.nn.Module], data_loader: Iterable,
                    ...
                    return_all_layers=False, standardize_depth=True, log_images=False, fp16=True):
    model.train()
...
@torch.no_grad() #REF : evaluate function
def evaluate(model, tasks_loss_fn, data_loader, device, epoch, in_domains, num_classes,
             log_images=False, mode='val', return_all_layers=False, standardize_depth=True, fp16=True):
    ...
    for x in metric_logger.log_every(data_loader, print_freq, header):
        ...
        # Mask invalid input values
        for task in input_dict: #not used
            if task in ['rgb']:
                continue
            channels = input_dict[task].shape[1]
            input_dict[task][~tasks_dict['mask_valid'].repeat_interleave(repeats=channels, dim=1)] = 0.0
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            for task in preds: 
                task_losses = {
                    task: tasks_loss_fn[task](preds[task], tasks_dict[task])
                }
                if(task == 'semseg'):
        ...
        metrics = masked_nyu_metrics(preds['depth'], tasks_dict['depth'], mask_valid=None)
        scores = compute_metrics(seg_preds, seg_gts, size=len(data_loader.dataset), num_classes=num_classes,
...
def compute_metrics(seg_preds, seg_gts, size, num_classes, device, ignore_index=utils.SEG_IGNORE_INDEX, dist_on='cpu'):
